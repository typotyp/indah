...
def banner():
	os.system("clear")
	prints(Panel(f"{P2}\tSELAMAT DATANG DI TOLS {color_rich}'R4MX'{P2} MULTI BRUTE FORCE IG",width=80,padding=(0,4),style=f"{color_table}"))
	prints(Panel(f'''{O2}                  "PREMIUM | RAMAXD SENGGOL BACOK"{color_rich}
 │
 │         ██████╗  █████╗ ███╗   ███╗██╗  ██╗██████╗      │
 │         ██╔══██╗██╔══██╗████╗ ████║╚██╗██╔╝██╔══██╗     │
 │         ██████╔╝███████║██╔████╔██║ ╚███╔╝ ██║  ██║     │
 ...
 │         ██║  ██║██║  ██║██║ ╚═╝ ██║██╔╝ ██╗██████╔╝     │
 │         ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝╚═════╝                         
 {H2}•{P2} Author : RamaXD{color_rich}                                       
...
class instagram:
	...
	def menu(self):
		...
		if c=='':
			prints(Panel(f"{P2}pilih yang bener goblok !",width=80,padding=(0,19),style=f"{color_table}"));time.sleep(3);exit()
		elif c in ('1','01'):
...
def tlisensi():
    ...
    if lisen in ['']:
     ...
     tlisensi()
    if lisen in ['buy','Buy']:
     os.system('xdg-open https://wa.me/6283862128349?text=Bang+beli+lisensi+IG+nya+dong');exit()
    loadinglisen()
